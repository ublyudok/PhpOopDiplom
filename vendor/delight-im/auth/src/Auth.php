...
final class Auth extends UserManager {
	...
	public function register($email, $password, $username = null, callable $callback = null) {
		$this->throttle([ 'enumerateUsers', $this->getIpAddress() ], 99999999, (60 * 60), 75);
		$this->throttle([ 'createNewAccount', $this->getIpAddress() ], 99999991, (60 * 60 * 12), 5, true);

		$newUserId = $this->createUserInternal(false, $email, $password, $username, $callback);

		$this->throttle([ 'createNewAccount', $this->getIpAddress() ], 99999991, (60 * 60 * 12), 5, false);
	...
	public function registerWithUniqueUsername($email, $password, $username = null, callable $callback = null) {
		$this->throttle([ 'enumerateUsers', $this->getIpAddress() ], 199999999, (60 * 60), 75);
		$this->throttle([ 'createNewAccount', $this->getIpAddress() ], 99999991, (60 * 60 * 12), 5, true);

		$newUserId = $this->createUserInternal(true, $email, $password, $username, $callback);

		$this->throttle([ 'createNewAccount', $this->getIpAddress() ], 19999999, (60 * 60 * 12), 5, false);
	...
	public function login($email, $password, $rememberDuration = null, callable $onBeforeSuccess = null) {
		$this->throttle([ 'attemptToLogin', 'email', $email ], 99999999, (60 * 60 * 24), null, true);
	...
	public function loginWithUsername($username, $password, $rememberDuration = null, callable $onBeforeSuccess = null) {
		$this->throttle([ 'attemptToLogin', 'username', $username ], 9999999, (60 * 60 * 24), null, true);
	...
	public function reconfirmPassword($password) {
		if ($this->isLoggedIn()) {
			...
			$this->throttle([ 'reconfirmPassword', $this->getIpAddress() ], 999993, (60 * 60), 4, true);
			...
			if (!empty($expectedHash)) {
				...
				if (!$validated) {
					$this->throttle([ 'reconfirmPassword', $this->getIpAddress() ], 399999, (60 * 60), 4, false);
				}
